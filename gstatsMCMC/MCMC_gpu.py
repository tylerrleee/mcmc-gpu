"""
Docstring for gstatsMCMC.MCMC-gpu
Created: 02/17/2026

Refactored from MCMC.py -- niyashao
...
def spectral_synthesis_field(RF, shape, res=1.0, device: torch.device = None):
    """
    ...
    GPU-ported: frequency grids, spectral power, white noise, and inverse FFT are all
    computed via ``torch.fft`` on *device*.  The result is returned as a CPU NumPy array
    so that the rest of the pipeline (gstools, verde, etc.) continues to work unchanged.

    Args:
        RF (RandField): RandField configuration object.
        shape (tuple[int, int]): Output shape as ``(ny, nx)``.
        res (float, optional): Grid spacing (default 1.0).
        device (torch.device, optional): Compute device.  Falls back to the
            module-level ``DEVICE`` (CUDA ; MPS ; CPU) when not specified.
    ...
    kx = torch.fft.fftfreq(nx, d=res, device=device) * (2.0 * math.pi)  # (nx,)
    ky = torch.fft.fftfreq(ny, d=res, device=device) * (2.0 * math.pi)  # (ny,)

    kyv, kxv = torch.meshgrid(ky, kx, indexing="ij")  # shapes: (ny, nx)
    k = torch.sqrt(kxv ** 2.0 + kyv ** 2.0) + 1e-10        # (ny, nx)

 
    # Spectral power density
    ## we use np.sqrt over torch.sqrt b/c the loss functions utilizes CPU based library
    ...
    if model_name == "Gaussian":
        ...
        constant = (4.0 * np.pi * math.gamma(nu + 1) * (2 * nu)**nu) / (math.gamma(nu) * a**(2*nu))
        keppa = 2*nu / (a**2)
        S = constant * ((keppa + 4 * np.pi * k**2) ** (-nu - 1))

    # Complex white noise
    noise_real = torch.randn(ny, nx, device=device)
    noise_imag = torch.randn(ny, nx, device=device)
    noise      = torch.complex(noise_real, noise_imag)   # complex64 tensor

    #noise = np.random.normal(size=(ny, nx)) + 1j * np.random.normal(size=(ny, nx))
    freq_field = noise * torch.sqrt(S.to(noise.real.dtype))  

    # Inverse FFT
    field = torch.fft.ifft2(freq_field).real  # (ny, nx) float32
    field = (field - field.mean()) / (field.std() + 1e-12)

    # replaces rng.normal(0, np.sqrt(nug), size=(ny, nx))
    nugget_noise = torch.randn(ny, nx, device=device) * torch.sqrt(nug)

    # Apply scaling and nugget noise
    final_field = field * scale + nugget_noise
    #field = field * scale + np.random.normal(0, np.sqrt(nug), size=(ny, nx))

    ## Return as CPU numpy array – helps downstream code 
    ## (gstools, verde, gstats, scikit, etc,..) expects numpy
    return final_field
...
class RandField:
    ...
    def get_edge_masks(self):
        ...
        pairs = self.pairs
        res = self.resolution
        L, x0, k, offset = self.logistic_param
        for i in range(pairs.shape[1]):
            ...
            dist_t    = torch.tensor(dist_edge, dtype=torch.float32, device=device)
            # re-scale the distance by the maximum correlation distance
            dist_rescale_edge = RandField.rescale(dist_t, self.max_dist)
            # calculate the logistic function
            dist_logi_edge = RandField.logistic(dist_rescale_edge,  L, x0, k) - offset

            edge_masks.append(dist_logi_edge.cpu().numpy())
    ...
    def min_dist_old(hard_mat, xx, yy):
        """
    ...
    def min_dist(hard_mat, xx, yy, device: torch.device = None, batch_size:int = 10000):
        """
        Compute the minimum Euclidean distance from every grid cell to the nearest
        non-NaN point in *hard_mat*.

        Algorithm
        ---------
        1. Identify the set of *source* points where ``hard_mat`` is **not** NaN.
           These are the locations we measure distance *from*.
        2. Treat every grid cell as a *query* point.
        3. Compute pairwise Euclidean distances with ``torch.cdist`` → shape
           ``(n_queries, n_sources)``.
        4. Take the column-wise minimum -> one distance per query.
        5. Reshape back to the original grid shape and return as a CPU NumPy array.

        Notes: This is an internal helper function and not intended for direct use.
        """
        H, W = xx.shape

        # non_NaN mask -- source points
        mask_hard = ~np.isnan(hard_mat) # (H, W) bool
        src_x = torch.tensor(xx[mask_hard],  dtype=torch.float32, device=device)  # (S,)
        src_y = torch.tensor(yy[mask_hard],  dtype=torch.float32, device=device)  # (S,)
        sources = torch.stack([src_x, src_y], dim=1)  # (S, 2)
       
       # If no poitns exist in sources | return inf distance
        if sources.shape[0] == 0:
            return np.full((H, W), np.inf)
    
        # Query points -- all grid cells
        qry_x   = torch.tensor(xx.ravel(), dtype=torch.float32, device=device)    # (N,)
        qry_y   = torch.tensor(yy.ravel(), dtype=torch.float32, device=device)    # (N,)
        queries = torch.stack([qry_x, qry_y], dim=1)  # (N, 2)

        # Find Minimum euclidean distance at p=2 | improvement: batching
        N = queries.shape[0] # number of grids
        min_dists = torch.empty(N, dtype=torch.float32, device=device) # empty array to populate

        # Reduce VRAM usage by batching
        # Complexity: batch-size * S * 4 bytes 
        for i in range(0, N, batch_size):
            # Slice current batch of query points
            batch_queries = queries[i : i + batch_size]

            # Compute pairwise distances for this batch only: (Batch_Size, S)
            dists = torch.cdist(batch_queries, sources, p=2.0)

            # Find min distance for each query in the batch 
            batch_mins, _ = dists.min(dim=1)

            # Store result and free 'dists' memory
            min_dists[i : i + batch_size] = batch_mins

        return min_dists.reshape(H, W).cpu().numpy()
    
    def rescale(x, maxdist):
        """
        Rescale distance values by the specified maximum distance.
        - Accounts for numpy and tensor.
        Notes: This is an internal helper function and not intended for direct use.
        """
        if isinstance(x, torch.Tensor):
            return torch.where(x > maxdist, torch.ones_like(x), x / maxdist)
    ...
    def get_crf_weight(self,xx,yy,cond_data_mask):
        ...
        # re-scale the distance by the maximum correlation distance
        dist_rescale_t = RandField.rescale(dist_t, max_dist)
        # calculate the logistic function
        dist_logi_t    = RandField.logistic(dist_rescale_t, L, x0, k) - offset
        weight_t       = dist_logi_t - torch.min(dist_logi_t)

        dist_rescale = dist_rescale_t.cpu().numpy()
        dist_logi    = dist_logi_t.cpu().numpy()
        weight       = weight_t.cpu().numpy()
    ...
    def get_crf_weight_from_dist(self,xx,yy,dist):
        ...
        dist_rescale_t = RandField.rescale(dist_t, max_dist)
        dist_logi_t    = RandField.logistic(dist_rescale_t, L, x0, k) - offset
        weight_t       = dist_logi_t - dist_logi_t.min()
...
class chain:
    ...
    def __init__(self, xx, yy, initial_bed, surf, velx, vely, dhdt, smb, cond_bed, data_mask, grounded_ice_mask, resolution):
        ...
        Inputs may be either ``numpy.ndarray`` or ``torch.Tensor``.  All
        simulation grids are stored internally as ``torch.float32`` tensors on
        *device*.  Integer / binary masks (``data_mask``, ``grounded_ice_mask``)
        are stored as ``torch.int32`` tensors on *device* to save memory.

        Args:
            xx (np.ndarray or torch.Tensor)     : A 2D array of grid x-coordinates.
            yy (np.ndarray or torch.Tensor)     : A 2D array of grid y-coordinates.
            initial_bed (np.ndarray or torch.Tensor): The initial bed topography guess.
            surf (np.ndarray or torch.Tensor)   : The ice surface elevation.
            velx (np.ndarray or torch.Tensor)   : The ice surface velocity in the x-direction.
            vely (np.ndarray or torch.Tensor)    The ice surface velocity in the y-direction.
            dhdt (np.ndarray or torch.Tensor)   : The rate of surface height change.
            smb (np.ndarray or torch.Tensor)    : The surface mass balance.
            cond_bed (np.ndarray or torch.Tensor): Conditioning radar bed measurements.
                - Grid cells without data should be marked with NaN.
            data_mask (np.ndarray or torch.Tensor): A binary mask where 1 indicates
                - the presence of conditioning data.  Stored as ``torch.int32``.
            grounded_ice_mask (np.ndarray or torch.Tensor): A binary mask where 1
                - indicates grounded ice.  Stored as ``torch.int32``.
            resolution (float): The spatial resolution of the grid in meters.
            device (torch.device, optional): The compute device to use.  
                - Defaults to the module-level ``device`` chosen at import time 
                - e.g. CUDA ; MPS ; CPU
        ...
        # HELPERs: Standardize dtype across all arguments
        # 1. if it's a tensor -> convert ; 2. if not, turn into tensor -> convert
        def _to_float32(arr):
            """Convert numpy array or torch.Tensor to float32 tensor on device."""
        ...
        def _to_int32(arr):
            """Convert numpy array or torch.Tensor to int32 tensor on device."""

            if isinstance(arr, torch.Tensor):
                return arr.to(dtype  = torch.int32, 
                              device = device)
        ...
        # Physical Constraint shape check 
        arrays_to_check = [surf, velx, vely, dhdt, smb, cond_bed, data_mask]

        # Check if any array's shape doesn't match initial_bed
        if any(arr.shape != initial_bed.shape for arr in arrays_to_check):
            raise Exception('the shape of bed, surf, velx, vely, dhdt, smb, radar_bed, data_mask need to be same')


        # --- float32 simulation grids ---
        self.xx          = _to_float32(xx)
        self.yy          = _to_float32(yy)
        self.initial_bed = _to_float32(initial_bed)
        self.surf        = _to_float32(surf)
        self.velx        = _to_float32(velx)
        self.vely        = _to_float32(vely)
        self.dhdt        = _to_float32(dhdt)
        self.smb         = _to_float32(smb)
        self.cond_bed    = _to_float32(cond_bed)

        # --- int32 masks (binary; no floating-point math needed) ---
        self.data_mask         = _to_int32(data_mask)
        self.grounded_ice_mask = _to_int32(grounded_ice_mask)
    ...
    def set_update_region(self, update_in_region, region_mask = []):
        ...
        Args:
            ...
            region_mask (np.ndarray or torch.Tensor): A binary mask specifying the update region.
                - Converted to ``torch.int32`` and placed on ``self.device``.
        ...
        if update_in_region == False:
            ...
            self.region_mask = torch.ones(self.xx.shape, 
                                          dtype  = torch.int32, 
                                          device = self.device)
            
        else:
            if region_mask.shape != tuple(self.xx.shape):
                raise ValueError('the region_mask input is invalid. It has to be a 2D array with the shape of the map')
                ...
                if isinstance(region_mask, torch.Tensor):
                    self.region_mask = region_mask.to(dtype  = torch.int32, 
                                                      device = self.device)
                else:
                    self.region_mask = torch.tensor(region_mask, 
                                                    dtype  = torch.int32, 
                                                    device = self.device)
    

    def __meanabs(data: torch.Tensor, mask: torch.BoolTensor) -> float:
        """
        A loss function, return the mean of absolute values of data inside the mask
        
        Args:
            data (torch.Tensor): 2-D float32 field on the chain's device.
            mask (torch.BoolTensor): Boolean selection mask (True = include).

        Returns:
            the mean of absolute data inside the mask
        """
        return torch.nanmean(torch.abs(data[mask == 1]))
    
    def __meansq(data: torch.Tensor, mask: torch.BoolTensor) -> float:
        """
        ...
        """
        return torch.nanmean(torch.square(data[mask == 1]))
    
    def __sumabs(data: torch.Tensor, mask: torch.BoolTensor) -> float:
        """
        ...
        """
        return torch.nansum(torch.abs(data[mask == 1]))
    ...
    def __sumsq(data,mask):
        ...
        """
        return torch.nansum(torch.square(data[mask == 1]))
    
    def __return0(data: torch.Tensor, mask: torch.BoolTensor) -> float:
        return 0.0
...
# =============================================================================
        if massConvInRegion:
            ...
            self.mc_region_mask = torch.ones(self.xx.shape, 
                                             dtype  = torch.int32, 
                                             device = self.device)
...
# =============================================================================
    ...
    def loss(self, massConvResidual, dataDiff):
        ...
        Args:
            massConvResidual (np.ndarray or torch.Tensor): A 2D array of the mass flux residuals field.
            dataDiff (np.ndarray or torch.Tensor)        : A 2D array of the difference between the candidate
                topography and observed bed elevation. For multiprocessing, can use any placeholder.
        ...
        """
        # Type check
        if not isinstance(massConvResidual, torch.Tensor):
            massConvResidual = torch.tensor(massConvResidual,
                                            dtype  = torch.float32, 
                                            device = self.device)
        else:
            massConvResidual = massConvResidual.to(dtype  = torch.float32, 
                                                   device = self.device)
        ...
        # Boolean mask
        residuals_in_region = massConvResidual[self.mc_region_mask == 1]

        # SSE based on Gaussian basically
        loss_mc = torch.nansum(residuals_in_region ** 2).item() / (2 * self.sigma_mc ** 2)
...
class chain_crf(chain):
    ...
    def set_crf_data_weight(self, RF):
        ...
        # Pass as numpy array 
        _np = lambda t: t.cpu().numpy() if isinstance(t, torch.Tensor) else t

        crf_weight, dist, dist_rescale, dist_logi = RF.get_crf_weight(_np(self.xx), 
                                                                      _np(self.yy), 
                                                                      _np(self.data_mask))
        self.crf_data_weight = torch.tensor(crf_weight, 
                                            dtype   = torch.float32, 
                                            device  = device
                                    )
    ...
    def run(self, n_iter, RF, only_save_last_bed=False, info_per_iter = 1000, plot=True, progress_bar=True):
        ...
        step_cache      = np.zeros(n_iter, dtype=bool)
        blocks_cache    = np.full((n_iter, 4), np.nan)
        resampled_times = np.zeros(self.xx.shape)

        # bed_cache lives in CPU RAM (pinned memory avoids extra copy on CUDA)
        if not only_save_last_bed:
            bed_cache = torch.zeros(
                (n_iter, H, W),
                dtype  = torch.float32,
                device = 'cpu',
                pin_memory = (device == 'cuda'),   # bool : speedup on CUDA hosts if exist
            )
        ...
        if not (self.sample_loc is None):
            ...
            sample_loc_ij = np.zeros(self.sample_loc.shape, dtype = np.int32)
            for k in range(self.sample_loc.shape[0]):
                sample_i,sample_j = np.where((self.xx == self.sample_loc[k , 0]) & 
                                             (self.yy == self.sample_loc[k , 1])
                                             )
                sample_loc_ij[k,:] = [int(sample_i[0]), int(sample_j[0])]
                
            sample_values[:,0] = self.initial_bed[sample_loc_ij[:,0],sample_loc_ij[:,1]].cpu().numpy()
            
        # Avoid in-place modification
        bed_c: torch.Tensor = self.initial_bed.clone()

        resolution = self.resolution
        
        # initialize loss
        mc_res = Topography_gpu.get_mass_conservation_residual(bed_c, self.surf, self.velx, self.vely, self.dhdt, self.smb, resolution)
        ...
        step_cache[0]      = False
        if not only_save_last_bed:
            bed_cache[0].copy_(bed_c, non_blocking=True)   # async (D, H) copy
        ...
        for i in iterator:
            ...
            f: torch.tensor       = RF.get_rfblock() # CPU numpy
            block_size = f.shape       # (bH, bW)
            ...
            if self.update_in_region:
                while True:
                    ...
                    if self.region_mask[indexx, indexy]:  # bool tensor
                        break
            else:
                indexx = int(rng.integers(0, H))
                indexy = int(rng.integers(0, W))
            ...
            # Block-local offset (handles map-edge clipping)
            ...
            if self.update_in_region:
                bed_next = torch.where(self.region_mask, bed_next, bed_c)
            else:
                bed_next = torch.where(self.grounded_ice_mask, bed_next, bed_c)

            # Define Padded Block to solve gradient & mass conservation residual (MSR)
            pad     = 1 
            c_xmin = np.max(0, bxmin - pad) # Left boundary
            c_xmax = np.min(H, bxmax + pad) # Right Boundary
            c_ymin = np.max(0, bymin - pad) # Floor Boundary
            c_ymax = np.min(W, bymax + pad) # Ceiling Boundary
            ...
            # Some Float32 on Tensor
            local_mc_res = Topography_gpu.get_mass_conservation_residual(local_bed, local_surf, local_velx, local_vely, local_dhdt, local_smb, resolution)   
            ...
            if (u <= acceptance_rate):
                ...
                # region mask or grounded ice mask depending on whether update in region is true
                mask_slice = block_region_mask.cpu().numpy()
                resampled_times[bxmin:bxmax, bymin:bymax] += mask_slice
            ...
            if not only_save_last_bed:
                bed_cache[i].copy_(bed_c, non_blocking=True)

                
            if not (self.sample_loc is None):
                sample_values[:,i] = bed_c[sample_loc_ij[:,0],sample_loc_ij[:,1]].cpu().numpy()
        ...
        last_bed = bed_c.detach().cpu().numpy()
...
class chain_sgs(chain):
    ...
    def run(self, n_iter, only_save_last_bed=False, info_per_iter=100, plot=True, progress_bar=True):
        ...
        if self.detrend_map == True:
            mc_res = Topography_gpu.get_mass_conservation_residual(bed_c + self.trend, self.surf, self.velx, self.vely, self.dhdt, self.smb, resolution)
        else:
            mc_res = Topography_gpu.get_mass_conservation_residual(bed_c, self.surf, self.velx, self.vely, self.dhdt, self.smb, resolution)
        ...
        for i in range(n_iter):
            ...
            if self.detrend_map == True:
                mc_res = Topography_gpu.get_mass_conservation_residual(bed_next + self.trend, self.surf, self.velx, self.vely, self.dhdt, self.smb, resolution)
            else:
                mc_res = Topography_gpu.get_mass_conservation_residual(bed_next, self.surf, self.velx, self.vely, self.dhdt, self.smb, resolution)
