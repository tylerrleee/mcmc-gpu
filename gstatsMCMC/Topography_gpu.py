...
def get_mass_conservation_residual(bed, surf, velx, vely, dhdt, smb, resolution):    
    ...
    bed_t  = to_tensor(bed,  device=device)
    surf_t = to_tensor(surf, device=device)
    velx_t = to_tensor(velx, device=device)
    vely_t = to_tensor(vely, device=device)
    dhdt_t = to_tensor(dhdt, device=device)
    smb_t  = to_tensor(smb,  device=device)

    thick_t = surf_t - bed_t

    # torch.gradient returns a list; axis 1 = x (columns), axis 0 = y (rows)
    # spacing argument corresponds to the physical resolution
    (dx_t,) = torch.gradient(velx_t * thick_t, spacing=(float(resolution),), dim=1)
    (dy_t,) = torch.gradient(vely_t * thick_t, spacing=(float(resolution),), dim=0)

    res_t = dx_t + dy_t + dhdt_t - smb_t
    return to_numpy(res_t)
